...
public class CreditCardType {
    ...
    public static void main(String[] args){
        ...
        Dataset<Row> dragonCard_low = spark.read()
                .jdbc(prop.getProperty("url"),"tbldragoncard","dragoncardid", 1, 300000000, 200, prop)
                .select("agentid", "dragoncode", "balancetype").na().drop()
                .filter("dragoncardid < 300000000 and agentid != -1");
        dragonCard_low.show(10);
        Dataset<Row> dragonCard_Up = spark.read()
                .jdbc(prop.getProperty("url"),"tbldragoncard","dragoncardid", 300000000, 2978724611L, 200, prop)
                .select("agentid", "dragoncode", "balancetype").na().drop()
                .filter("dragoncardid > 300000000 and agentid != -1");
        dragonCard_Up.show(10);
        Dataset<Row> dragonCard = dragonCard_low.union(dragonCard_Up);
        dragonCard.show(10);

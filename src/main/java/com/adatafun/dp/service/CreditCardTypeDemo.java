...
/**
 * CreditCardTypeDemo.java
 * Copyright(C) 2017 杭州风数科技有限公司
 * Created by wzt on 2018/3/21.
 */
public class CreditCardTypeDemo {
    ...
    public static void main(String[] args){
        ...
        Dataset<Row> bindRecord = spark.read()
                .jdbc(prop.getProperty("url"),"tb_bindrecord", "id", 1, 3686000, 100, prop)
                .withColumnRenamed("dragoncode", "dragon_code")
                .select("user_id", "dragon_code").na().drop();
        bindRecord.show(10);
        Dataset<Row> dragonCard = spark.read()
                .jdbc(prop.getProperty("url"),"tbldragoncard","dragoncardid", 1, 29725601, 100, prop)
                .select("agentid", "dragoncode", "balancetype").na().drop()
                .filter("dragoncardid < 29725601 and agentid != -1");
        dragonCard.show(10);
        ...
        System.out.println(tupleRDD.take(100));
